...
LOO, WAIC, and the like cannot handle latent variables, as we have no observations to compare them to. As such, we need to specify what response variables we are interested in. For example:

```{r waic_latent, warning = FALSE}
waic(mod_spartina,
    resp = c('clonediams', 'numbstemss', 
             'numbinfls', 'meanhts', 'meanwidths'))
```
...
2025-01-08: Added WAIC for LV models.
